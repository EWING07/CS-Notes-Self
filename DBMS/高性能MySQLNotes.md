...
1. 索引基础
   - 索引是在存储引擎层而不是服务器层实现的。
   - B-Tree索引：
     - ​	底层的存储引擎也可能使用不同的存储结构，例如：InnoDB使用的是B+ Tree
     - MyISAM索引通过数据的物理位置引用被索引的行，而InnoDB则根据主键引用被索引的行。
     - B-Tree索引适用于全键值、键值范围或键前缀查找。
   - 哈希索引：
     - 基于hasha table实现，只有精确匹配索引所有列的查询才有效。对于每一行数据，存储引擎都会对所有的索引列计算出一个哈希码，hash code是一个比较小的值，并且不同的键值的行计算出来的哈希码也不一样。hash index将索引的哈希码存储在索引中，同时在哈希表中保存指向每个数据行的指针。
     - 只有memory引擎显式支持哈希索引，还支持非唯一的hash index，
     - 不支持任何范围查询，
     - InnoDB引擎：adaptive hash index。当innoDB注意到某些index使用很频繁的时候，会在内存中基于b-tree索引之上在创建一个hash index，这样就让B-tree索引也具有哈希索引的一些优点，比如快速哈希查找。
     - 重点：如果想避免冲突问题，必须在WHERE条件中带入哈希值和对应列值。可以使用FNV64()函数：速度比CRC32()
   - 空间数据索引：
     - MyISAM表支持空间索引，可以用作地理数据存储。与B-Tree索引不一样，不要前缀查询，从所有维度来索引数据。
     - PostogreSQL和PostGIS做的比较好
   - 全文索引
     - 查找文本关键词，类似于搜索引擎做的事情，而不是简单的WHERE条件匹配。
     - 适用于MATCH AGAINST操作，而不是普通的WHERE条件操作。

2. 索引的优点

   - B-Tree索引，按照顺序存储数据，所以MySQL可以用来做ORDER BY和GROUP BY操作。因为数据是有序的，索引B-Tree会将相关的列值都存储在一起。
   - 三个优点：
     - 大大减少了服务器需要扫描的数据量
     - 索引可以帮助服务器避免排序和临时表
     - 索引可以将随机I/O变为顺序I/O
...
3. 高性能的索引策略
   ...
   - **前缀索引和索引的选择性：**对于很长的字符列，需要索引开始的部分字符，对于BLOB、TEXT或者很长的VARCHAR的列，必须使用前缀索引，因为MySQL不允许索引这些列的完整长度。

   - **多列索引：**可以使用看EXPLAIN中的Extra列看到嵌套操作。索引合并策略有时候是一种优化的结果，但是实际上更多时候说明了表上的索引建得很糟糕：

     - 多个AND条件: 需要一个包含索引相关列的多列索引，而不是多个独立的单列索引
     - 多个OR条件

   - **选择合适的索引列顺序：**经验法则：考虑全局基数和选择性，而不是某个具体查询。

   - **聚簇索引：**插入主键使用AUTO_INCREMENT自增列。不要使用UUID，性能不好。

     ![image-20201120172609730](image-20201120172609730.png)

   - 覆盖索引：如果一个索引包含所有需要查询的字段的值，我们就称之为“覆盖索引”
     - 索引条目通常小于数据行大小，如果只需要读取索引，那么mysql就会极大的的减少数据访问量，这对于缓存的负载非常重要。
     - index condition pushdown，mysql 5.6
   - 使用索引扫描来做排序
     - 生成有序结果的两种方法：1、排序操作 2. 索引顺序扫描
   - ![image-20201127152359307](image-20201127152359307.png)
